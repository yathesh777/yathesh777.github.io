...
const AI_ENDPOINT = (window.YK_AI_ENDPOINT || '').trim();
const AI_MODEL_LABEL = (window.YK_AI_MODEL_LABEL || 'Gemini').trim();
...
(() => {
  ...
  // YK AI copilot. Uses a secure backend when configured, with a local fallback if the backend is unreachable.
  const copilot = document.querySelector('#copilot'); const copilotToggle=document.querySelector('#copilot-toggle'); const copilotForm=document.querySelector('#copilot-form'); const copilotInput=document.querySelector('#copilot-query'); const copilotLog=document.querySelector('#copilot-log'); const copilotStatus=document.querySelector('#copilot-status');
  ...
  if (copilotStatus) copilotStatus.textContent = AI_ENDPOINT ? `● ${AI_MODEL_LABEL} live` : '● Demo mode';
  ...
  async function answer(q){ if(!AI_ENDPOINT) return {answer:answerLocal(q),live:false}; try{ const r=await fetch(AI_ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:q})}); const d=await r.json().catch(()=>({})); if(!r.ok) throw new Error(d.detail||'AI endpoint failed'); return {answer:d.answer||answerLocal(q),live:true}; } catch(err) { return {answer:`The live AI service is temporarily unavailable. ${answerLocal(q)}`,live:false}; } }
  const pushMsg=(text,who,meta='')=>{ if(!copilotLog)return; const wrap=document.createElement('div'); wrap.className=`copilot-msg-wrap ${who}`; const d=document.createElement('div'); d.className=`copilot-msg ${who}`; d.textContent=text; wrap.appendChild(d); if(meta){ const m=document.createElement('div'); m.className='copilot-meta mono'; m.textContent=meta; wrap.appendChild(m); } copilotLog.appendChild(wrap); copilotLog.scrollTop=copilotLog.scrollHeight; };
  async function ask(q){ const query=q.trim(); if(!query)return; pushMsg(query,'user'); if(copilotInput)copilotInput.value=''; const sendBtn=copilotForm?.querySelector('button'); sendBtn?.setAttribute('disabled','true'); if(copilotStatus)copilotStatus.textContent=AI_ENDPOINT?`◌ ${AI_MODEL_LABEL} thinking`:'● Demo mode'; const result=await answer(query); pushMsg(result.answer,'bot',result.live?`LIVE · ${AI_MODEL_LABEL}`:'DEMO FALLBACK'); if(sendBtn)sendBtn.removeAttribute('disabled'); if(copilotStatus)copilotStatus.textContent=AI_ENDPOINT?`● ${AI_MODEL_LABEL} live`:'● Demo mode'; }
