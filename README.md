...
## YK AI — live Gemini mode

The portfolio includes a real Gemini-powered assistant through the `backend/` FastAPI service. GitHub Pages remains the static frontend; the backend keeps the Gemini API key server-side.

1. Create a Gemini API key in Google AI Studio.
2. Run/deploy `backend/` with `GEMINI_API_KEY` set.
3. Set `window.YK_AI_ENDPOINT` in `assets/ai-config.js` to the deployed `/api/chat` URL.
4. Keep the API key out of Git, HTML, CSS, and frontend JavaScript.

Without a configured backend, the UI falls back to the portfolio's local demo responses.
